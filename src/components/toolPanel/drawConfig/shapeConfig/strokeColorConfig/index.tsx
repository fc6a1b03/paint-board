...
const StrokeColorConfig = () => {
  const {
    ...
    updateStrokeColor,
    deleteStrokeColor,
    updateCurrentStrokeColor
  } = useShapeStore()
  const { t } = useTranslation()
  const [editColorIndex, setEditColorIndex] = useState(-1)

  const handleBorderColor = _.debounce((color: string, colorIndex: number) => {
  ...
  return (
    <div className="form-control mt-3">
      ...
      <div className="mt-2 pb-1 flex items-center gap-x-2 w-full relative">
        {strokeColorList.map((strokeColor, strokeColorIndex) => (
          <div
            className={`relative w-6 h-6 transition-all duration-300 ${
              currentStrokeColor === strokeColorIndex ? 'w-7' : ''
            }`}
            key={strokeColor + strokeColorIndex}
          >
            <div
              className={`absolute top-0 left-0 shrink-0 flex items-center justify-between w-6 bg-gray-200 overflow-hidden join hover:w-20 hover:z-10 transition-all duration-300 ring-offset-1 ${
                currentStrokeColor === strokeColorIndex
            ...
            >
              <div
                ...
                onClick={() => {
                  updateCurrentStrokeColor(strokeColorIndex)
                }}
              ></div>
              <label
                htmlFor="draw-color-input"
                onClick={() => {
                  setEditColorIndex(strokeColorIndex)
                }}
              >
                <EditIcon
              ...
              <ClearIcon
                ...
                onClick={() => {
                  deleteStrokeColor(strokeColorIndex)
                }}
              />
            </div>
          </div>
        ))}
        {strokeColorList.length < 6 && (
          <AddColorIcon
            className="w-6 h-6 cursor-pointer hover:opacity-60"
            onClick={() => {
              updateStrokeColor('#000000', strokeColorList.length)
            }}
          />
        )}
        <input
          ...
          type="color"
          value={
            editColorIndex >= 0 ? strokeColorList[editColorIndex] : '#000000'
          }
          onChange={(e) => {
            if (editColorIndex >= 0) {
              handleBorderColor(e.target.value, editColorIndex)
            }
          }}
